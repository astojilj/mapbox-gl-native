...
TEST(Transform, Padding) {
    ...
    const LatLng upperHalfScreenCenter = transform.screenCoordinateToLatLng({
        1000.0 / 2.0,
        1000.0 / 4.0,
    });
    ...
    ASSERT_DOUBLE_EQ(screenCenter.latitude(), paddedLowerHalfScreenCenter.latitude());
    ASSERT_NEAR(screenCenter.longitude(), paddedLowerHalfScreenCenter.longitude(), 1e-10);
    ASSERT_NEAR(upperHalfScreenCenter.latitude(), paddedScreenCenter.latitude(), 1e-10);
    ASSERT_NEAR(upperHalfScreenCenter.longitude(), paddedScreenCenter.longitude(), 1e-10);
}
